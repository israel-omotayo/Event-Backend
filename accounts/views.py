...
class GoogleAuthView(ProductionScopedThrottleMixin, APIView):
    permission_classes = [AllowAny]
    throttle_scope = "google_auth"

    @extend_schema(
        summary="Login or register with a verified Google ID token",
        request=GoogleAuthSerializer,
        responses={200: OpenApiResponse(description="JWT tokens issued.")},
    )
    def post(self, request):
